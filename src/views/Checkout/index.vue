<script setup>
import { getCheckoutList, createOrderAPI } from '@/apis/checkout.js';
...
// 激活项类名
// tab切换
//1. 点击时记录一个当前地址对象activeClass 点击哪个地址就让哪个地址记录下来
//2. 通过动态类名class 判断   条件为 当前id与点击的id是否相等
...
const updateAddress = () => {
    curAddress.value = activeClass.value;
...
<template>
    ...
    <el-dialog title="切换收货地址" width="30%" center v-model="toggleFlag">
        <div class="addressWrapper">
            <div class="text item" :class="{ active: item.id === activeClass.id }" @click="addActiveClass(item)"
